...
import { clearEroors, createNewSchool } from "../../actions/schoolsActions";
...
const NewSchool = () => {
  ...
  const { loading, error, success } = useSelector((state) => state.newSchool);
  ...
  useEffect(() => {
    ...
    if (success) {
      ...
      navigate("/dashboard");
      dispatch({ type: NEW_SCHOOL_RESET });
    }
  ...
  const createSchoolSubmitHandler = (e) => {
    ...
    dispatch(createNewSchool(myForm));
    console.log([...myForm]);
  };
